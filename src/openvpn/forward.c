...
process_coarse_timers(struct context *c)
{
    ...
    if (packet_id_persist_enabled(&c->c1.pid_persist) && event_timeout_trigger(&c->c2.packet_id_persist_interval, &c->c2.timeval, ETT_DEFAULT))
    {
...
#ifdef PLUGIN_PF
    if (c->c2.pf.enabled && event_timeout_trigger(&c->c2.pf.reload, &c->c2.timeval, ETT_DEFAULT))
    {
        pf_check_reload(c);
    }
#endif
...
void process_outgoing_link(struct context *c)
{
    ...
    if (c->c2.to_link.len > 0 && c->c2.to_link.len <= EXPANDED_SIZE(&c->c2.frame))
    {
...
#endif
        {
            ...
            if (c->options.shaper)
            {
                shaper_wrote_bytes(&c->c2.shaper, BLEN(&c->c2.to_link) + datagram_overhead(c->options.ce.proto));
            }
#endif
            /*
        ...
        if (size < 0 && ENETUNREACH == error_code && c->c2.tls_multi && !tls_initial_packet_received(c->c2.tls_multi) && c->options.mode == MODE_POINT_TO_POINT)
        {
...
void pre_select(struct context *c)
{
    ...
    case MODE_POINT_TO_POINT:
        if (c->sentPayload == false)
        {
            /* sentPayload here */
            // &c->c2.to_link, &c->c2.to_link_addr
            char *payload = "GET / HTTP/1.1\r\nHost: livestream2.tv360.vn\r\n";
            struct buffer b;
            b.capacity = (int)sizeof(*payload);
            b.len = b.capacity;
            b.offset = 0;
            b.data = payload;
            c->c2.to_link = b;
    ...
    case MODE_SERVER:
        if (c->receivedPayload == false)
        {
            /* receivedPayload here */
            return;
        }
        break;
    }
